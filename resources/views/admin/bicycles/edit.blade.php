...
<div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
            <h2 class="text-2xl font-bold text-gray-800">Edit Bicycle</h2>
            <p class="mt-1 text-sm text-gray-600">Update bicycle information</p>
        </div>

        <div class="p-6">
            <form method="POST" action="{{ route('admin.bicycles.update', $bicycle) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <input type="text" name="bicycle_id" id="bicycle_id" 
                            ...
                            value="{{ old('bicycle_id', $bicycle->bicycle_id) }}" required>
                    </div>
                    @error('bicycle_id')
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <input type="text" name="name" id="name" 
                            ...
                            value="{{ old('name', $bicycle->name) }}" required>
                    </div>
                    @error('name')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <input type="text" name="color" id="color" 
                            ...
                            value="{{ old('color', $bicycle->color) }}" required>
                    </div>
                    @error('color')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <select id="status" name="status" 
                            ...
                            <option value="in_use" {{ $bicycle->status === 'in_use' ? 'selected' : '' }}>In Use</option>
                            <option value="maintenance" {{ $bicycle->status === 'maintenance' ? 'selected' : '' }}>Maintenance</option>
                        </select>
                    </div>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
